...
export class ExportService {
     ...
     */
    async exportGroup(groupId: string, userId: string, options: ExportGroupDto): Promise<string> {
        // Fetch group data
        ...
        const statistics = options.includeStatistics
            ? {
                ...
                averageExpense:
                    expenses.length > 0
                        ? expenses.reduce((sum, exp) => sum + Number(exp.amount), 0) / expenses.length
                        : 0,
        ...
        const exportData: GroupExportData = {
            ...
            expenses: expenses.map((exp) => ({
                ...
                paidBy: {
                    name: `${exp.paidBy?.firstName || ''} ${exp.paidBy?.lastName || ''}`.trim() || 'Unknown',
                ...
                splits: exp.splits.map((split) => ({
                    user: {
                        name: `${split.user?.firstName || ''} ${split.user?.lastName || ''}`.trim() || 'Unknown',
                    },
     ...
     */
    async exportExpenses(userId: string, options: ExportExpensesDto): Promise<string> {
        // Build date filter
        const dateFilter: { expenseDate?: { gte?: Date; lte?: Date } } = {};
        if (options.startDate || options.endDate) {
            dateFilter.expenseDate = {};
     ...
     */
    async exportTransactions(userId: string, options: ExportTransactionsDto): Promise<string> {
        // Build date filter
