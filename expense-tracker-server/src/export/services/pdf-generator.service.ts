...
export class PdfGeneratorService {
    ...
    async generateGroupReport(data: GroupExportData): Promise<string> {
        const filename = `group-${data.group.id}-${Date.now()}.pdf`;
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                this.logger.error(`Error generating group report: ${error.message}`);
                reject(error);
            }
        });
    }
    ...
    async generateExpenseReport(data: ExpenseExportData): Promise<string> {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                this.logger.error(`Error generating expense report: ${error.message}`);
                reject(error);
            }
        });
    }
    ...
    async generateTransactionReport(
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                doc.moveDown(0.5);
                doc.fontSize(12);
                doc.fillColor('green').text(`Total Income: ₹${data.summary.totalIncome.toFixed(2)}`);
                doc.fillColor('red').text(`Total Expenses: ₹${data.summary.totalExpenses.toFixed(2)}`);
                doc.fillColor(data.summary.netSavings >= 0 ? 'green' : 'red')
